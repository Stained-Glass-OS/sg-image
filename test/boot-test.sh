...
case "${SG_GUEST_CHECK:-session}" in
    ...
    privilege)
        ...
        ;;
    elevated)
        # Elevated programs' displays (ADR 0012, bug B56): the interactive work
        # is host-side (below), driving the consent prompt and the installer
        # with the real keyboard. The guest check only waits for the prefix.
        CHECK_CMD="for i in \$(seq 1 $CHECK_TIMEOUT); do \
            [ -e /var/lib/stained-glass/prefix/.sg-initialized ] && break; sleep 1; done; \
            [ -e /var/lib/stained-glass/prefix/.sg-initialized ] \
              || { echo 'FAIL  prefix was still initializing'; exit 1; }; \
            echo 'PASS  prefix ready for the elevated-display gate'"
        CHECK_NAME="elevated-display gate (ADR 0012, B56)"
        ;;
    multiuser)
